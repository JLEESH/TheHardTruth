...
Note that the program also contains a scrambler using cubing notations. If you wish to visualise the cube state or the scrambling process a cuber would follow, please input the scramble generated by the program into the 'Moves' section of the following link (not made by me): 
...
Basically, a cuber would apply these moves to a cube, then time how long he or she takes to restore it back to the solved state. This removes the biases hand-scrambling a cube would have and ensures that the cuber solves a random state of the cube. You can also visualise the scrambling process using the intuitive controls on the lower left section of the webpage.
...
A file name 'times' containing some times I had lying around of some solves I did back in June/July 2014 was added for illustration. Click on 'Load Times' and look for the file in the relevant directories for an example on how the program functions.
